...
/**
 * Derives a 32-byte key from config.SECRET via SHA-256.
 * Works regardless of the secret's length or character set.
 */
function loadKey() {
    if (!config.SECRET) {
        throw new Error(
    ...
    return crypto.createHash('sha256').update(config.SECRET, 'utf8').digest();
}
...
function parse(token) {
    ...
    try {
        const decrypted = Buffer.concat([
            decipher.update(Buffer.from(cipherHex, ENCODING)),
            decipher.final(),
        ]);
...
/**
 * One-way SHA-256 hash. Useful for indexed lookups without storing plaintext.
 * @param {string} value
...
/**
 * Timing-safe string comparison. Use instead of === for tokens or hashes.
 * @param {string} a
 * @param {string} b
 * @returns {boolean}
 */
